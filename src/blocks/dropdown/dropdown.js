...
const createGuestDropdown = function (dataId, strings) {
  $(`.js-iqdropdown[data-id=${dataId}]`).iqDropdown({
    ...
    setSelectionText: (itemCount, totalItems) => {
      let guest;
      const isItemsSuitableForMultipleEnding = totalItems % 10 >= 5 || (totalItems >= 10 && totalItems <= 20);
      const isLastNumberOfGuestsBetweenTwoAndFour = totalItems % 10 >= 2 && totalItems % 10 <= 4;
      ...
      if (itemCount.babies > 0) {
        const isBabiesSuitableForMultipleEnding = itemCount.babies % 10 >= 5 || (itemCount.babies >= 10 && itemCount.babies <= 20);
        const isLastNumberOfBabiesBetweenTwoAndFour = itemCount.babies % 10 >= 2 && itemCount.babies % 10 <= 4;
  ...
  $('.js-iqdropdown[data-id=furniture]').iqDropdown({
    ...
    setSelectionText: (itemCount) => {
      ...
      if (itemCount.rooms !== 0) {
        const isLastNumberBetweenTwoAndFour = itemCount.rooms >= 2 && itemCount.rooms <= 4;
